...
function Terminal(rows, columns, Table) // constructor
{
	this.nrows = rows;
...
Terminal.prototype.Blink = function()
{
	...
	if (this.cursorvisible) this.color[this.cursory][this.cursorx] |= 0x600;
...
Terminal.prototype.PlotRow = function(row)
{
	...
	for(var i=0; i<this.ncolumns; i++)
	{
		...
		if (this.screen[row][i] == 0x0) line += "&nbsp;"; else 
		if (this.screen[row][i] == 0x20) line += "&nbsp;"; else
...
Terminal.prototype.HandleEscapeSequence = function()
{
	...
	if (this.escapestring == "[1K") // erase from start till cursor
	{
		this.DeleteArea(this.cursory, 0., this.cursory, this.cursorx);
		return;
	} else
	if (this.escapestring == "[K") // erase from cursor till end
	{
		this.DeleteArea(this.cursory, this.cursorx, this.cursory, this.ncolumns-1);
		return;
	}
	...
	if (s.charAt(0) != "[") return; // the short escape sequences must be handled earlier
	...
	if (numbers[0].length == 0) numbers = [];
	
	if (lastsign == "m") // colors
	{
		this.ChangeColor(numbers);
	} else
	if ((lastsign == "H") || (lastsign == "d")) // cursor
	{
		this.ChangeCursor(numbers);
	} else
	if ((lastsign == "G")) // change cursor column
	{
		this.cursorx = numbers[0];		
	} else
	if (lastsign == "A") // move cursor up
	{
		if (numbers.length == 0) this.cursory--; else this.cursory -= numbers[0]; 
	} else	
	if (lastsign == "E") // move cursor down
	{
		if (numbers.length == 0) this.cursory++; else this.cursory += numbers[0]; 
	} else	
	if (lastsign == "C") // move cursor right
	{
		if (numbers.length == 0) this.cursorx++; else this.cursorx += numbers[0]; 
	} else	
	if (lastsign == "D") // move cursor left
	{
		if (numbers.length == 0) this.cursorx--; else this.cursorx -= numbers[0]; 
	} else	
	if (lastsign == "X") // erase only number of characters in current line
	{
	...
	if (lastsign == "r") // set scrolling region, ignore
	{
	} else
	{
...
Terminal.prototype.PutChar = function(c)
{
	...
	if (this.escapetype == 2) // escape sequence (CS)
	{
	...
	if ((this.escapetype == 0) && (c == 0x1B)) // escape sequence
	{
		this.escapetype = 1;
		this.escapestring = "";
		return;
	...
	if (this.escapetype==1) // starting escape sequence
	{
		this.escapestring += String.fromCharCode(c);
		if (c == 0x5B) // Control Sequence Introducer ([)
		{
			this.escapetype = 2;
			return;
		}
...
TerminalInput.prototype.KeyPress = function(e)
{
	...
	if ((this.CTRLpressed) && ( ((key>=0x41) && (key<=0x5A)) || ((key>=0x61) && (key<=0x7A)) )) key &= 0x1F;
...
TerminalInput.prototype.KeyUp = function(e)
{
	...
	if (keycode == 17) this.CTRLpressed = false;
...
TerminalInput.prototype.KeyDown = function(e)
{
	...
	switch(keycode)
	{
		...
		case 17: // CTRL
			...
			return;
			break;
		case 67: // CTRL + c key handling for chrome
...
function Framebuffer() // constructor
{
...
Framebuffer.prototype.Update = function()
{
	...
	var i=3;
	for(; i<this.width*this.height*4; i+=4) this.buf8[i] = 0xFF;
...
function PrintState() 
{
	...
	for(var i=0; i<32; i+= 4) DebugMessage( 
			"   r"+(i+0) + ": " + hex8(cpu.r[i+0]) + 
			"   r"+(i+1) + ": " + hex8(cpu.r[i+1]) + 
			"   r"+(i+2) + ": " + hex8(cpu.r[i+2]) + 
			"   r"+(i+3) + ": " + hex8(cpu.r[i+3]) 
			);
	//if (cpu.jumpdelayed) DebugMessage("delayed jump");
	if (cpu.delayedins) DebugMessage("delayed instruction");

    if (cpu.SR_SM) DebugMessage("Supervisor mode"); else DebugMessage("User mode");
    if (cpu.SR_TEE) DebugMessage("tick timer exception enabled");
    if (cpu.SR_IEE) DebugMessage("interrupt exception enabled");
    if (cpu.SR_DME) DebugMessage("data mmu enabled");
    if (cpu.SR_IME) DebugMessage("instruction mmu enabled");
    if (cpu.SR_LEE) DebugMessage("little endian enabled");
    if (cpu.SR_CID) DebugMessage("context id enabled");
    if (cpu.SR_F) DebugMessage("flag set");
    if (cpu.SR_CY) DebugMessage("carry set");
    if (cpu.SR_OV) DebugMessage("overflow set");	
}
...
{  
	...
	req.onreadystatechange = function()
	{
		...
		if ((req.status != 200) && (req.status != 0)) { DebugMessage("Error: Could not load file " + url); return};
		var arrayBuffer = req.response;
		if (arrayBuffer) OnLoadFunction(arrayBuffer);
	}
	/*
...
function UART() // constructor
{
	...
	UART.prototype.ThrowCTI = function()
	{
		...
		if (!(this.IER & UART_IER_RDI)) return;
	...
	UART.prototype.ThrowTHRI = function()
	{
		...
		if (!(this.IER & UART_IER_THRI)) return;
	...
	{
		if ((this.ints & (1 << UART_IIR_CTI))  && (this.IER & UART_IER_RDI)) this.ThrowCTI(); else 
		if ((this.ints & (1 << UART_IIR_THRI)) && (this.IER & UART_IER_THRI)) this.ThrowTHRI(); else
		{
		...
		x &= 0xFF;
			...
			switch(addr)
			{
				case 0:
					...
					this.ThrowTHRI();
					break;
				case UART_IER: // 2 = 10b ,5=101b, 7=111b
				...
				case UART_FCR:
					...
					this.FCR = x;
					if (this.FCR&2) this.fifo = new Array(); // clear receive fifo buffer
					break;
				case UART_LCR:
					this.LCR = x;
					break;
...
function RAM(memsize) // constructor
{
	//use typed arrays
...
RAM.prototype.ReadMemory32 = function(addr)
{
	if (addr > this.memsize-4)
	{
		...
		{
			if (addr == 0x92000000) return ethreg0;
			if (addr == 0x92000038) 
			{			 
			var ret = ethreg38;
			if (ethreg38 == 0x1613) ethreg38 = 0xffff;
			if (ethreg38 == 0x22) ethreg38 = 0x1613;
			return ret;
			}
			return 0x0;
		} else
...
*/
	if (addr > this.memsize-4)
	{
		if ((addr >= 0x92000000) && (addr <= 0x92001000-4)) //eth
		{
			if (addr == 0x92000000) ethreg0 = x;
			return;
		} else
		if ((addr >= 0x91000000) && (addr <= 0x91101000-4)) //fb
		{
			if (addr == 0x91000014) fb.SetAddr(Swap32(x));
			//fb.addr = Swap32(x);
			//	fb.Update();
...
{
	//if (cpu.clock >= 0x001C6CB0) DebugMessage(hex8(addr) + " " + addr);
	...
	switch(addr&3)
	{
		case 0: return this.uint8mem[(addr&~3)|3];
		case 1: return this.uint8mem[(addr&~3)|2];
		case 2: return this.uint8mem[(addr&~3)|1];
		case 3: return this.uint8mem[(addr&~3)|0];
	}
...
{
    //if (this.SR_SM != ((x&1)?true:false)) DebugMessage("Supervisor: " + this.SR_SM);
    ...
    if (this.SR_LEE) {DebugMessage("little endian not supported"); abort();}
    if (this.SR_CID) {DebugMessage("context id not supported"); abort();}
    if (this.SR_EPH) {DebugMessage("exception prefix not supported"); abort();}
    if (this.SR_DSX) {DebugMessage("delay slot exception not supported"); abort();}
	if (this.SR_IEE && !old_SR_IEE) this.CheckForInterrupt();	
}
...
switch(group)
{
	...
	case 2: // ins MMU
		...
		return;
		break;
	case 3: // data cache, not supported
	case 4: // ins cache, not supported
		return;
		break;
	case 9: // pic
		switch(address)
		{
			case 0:
				...
				if (this.SR_IEE) // check immediate for interrupt
				if (this.PICMR & this.PICSR)
				{
	...
	case 10: //tick timer
		switch(address)
		{
			case 0:
				...
				if ((this.TTMR&0xFFFFFF)==(this.TTCR&0xFFFFFF)) this.TTMR &= 0x3FFFFFFF;
				break;
			case 1:
			default:
...
var r2=0x0, r3=0x0, r5=0x0, r4=0x0;
...
*/	
	...
	if (this.SR_SM) // check if supervisor mode
	{
		if
		(		
...
*/
	...
	if (this.SR_SM) // check if supervisor mode
	{
		if (!(tlbtr & 0x40)) // check if user read enable is not set(URE)
		{
	...
	{
		if (!(tlbtr & 0x80)) // check if supervisor read enable is not set (SRE)
		{	  
			this.Exception(EXCEPT_IPF, this.pc);
			return 0xFFFFFFFF;
		}
...
if ((steps&7)==0)	// do this not so often
{
	
// ---------- TICK ----------	
	if ((this.TTMR>>>30) != 0) // timer enabled
	{
		this.TTCR += 16;
		//this.TTCR++;
		...
		if ((this.TTCR&0xFFFFFF0) == (this.TTMR&0xFFFFFF0))
		{
			...
			if (this.TTMR&(1<<29)) // if interrupt enabled
			{
	...
	if ((this.SR_TEE) && (this.TTMR & (1<<28)))	// check if pending and check if interrupt must be triggered
	{
	...
	} else
	{
		...
		if (this.interrupt_pending) // test it here instead every time,
		{
			this.interrupt_pending = false;
			if ((this.PICSR) && (this.SR_IEE)) // check again because there could be another exception during this one cycle
			{
				this.Exception(EXCEPT_INT, this.group0[SPR_EEAR_BASE]);
				this.delayedins = false;
			}
...
if (!((pc^this.pc)&0xFFFFE000)) // short check if it is still the correct page
{
	pc = this.pc;
...
*/	
	...
	switch(ins >>> 26)
	{
		...
		case 0x6: // movhi or macrc
			rD = (ins>>>21)&0x1F;
			if (ins&0x10000) // if 16th bit is set
			{
...
//			imm |= (imm&0x8000)?0xFFFF0000:0;
		...
		case 0x38: // three operands commands
			...
			switch((ins >>> 0) & 0x3CF)
			{
				...
				case 0x10f: // fl1
					...
					for(var i=31; i>=0; i--)
						if (rA&(1<<i)) {r[rD] = i+1; break;}
					break;
				case 0x306: // mul signed (specification seems to be wrong)
					{
			...
			/*
					...
					*/
					...
					break;
				case 0x30a: // divu (specification seems to be wrong)
					//DebugMessage("divu signed not supported");
					//abort();				
					this.SR_CY = rB == 0;
					this.SR_OV = 0;
					if (!this.SR_CY) r[rD] = /*Math.floor*/(rA / rB);					
					break;
				case 0x309: // div (specification seems to be wrong)
					//DebugMessage("div not supported");
					//abort();					
					this.SR_CY = rB == 0;
					this.SR_OV = 0;
					if (!this.SR_CY) r[rD] =  int32(rA) / int32(rB);
...
//	for(var i=0; i<0x4000; i++)
	{
		//if (cpu.clock >= 1100001-1) DebugMessage(tracebufferindex);
		cpu.Step(0x10000);
//		fb.Update();
		//continue;
	/*
		if (tracebuffer[tracebufferindex] == undefined) {DebugMessage("tracebuffer undefined "+tracebufferindex); abort();}
		if (tracebuffer[tracebufferindex] != cpu.pc)
		{
			DebugMessage("Error: pc does not match " + hex8(tracebuffer[tracebufferindex]) + " " + hex8(cpu.pc));
			abort();
		}
		...
		for(var j=0; j<32; j++)
		{
			...
			{
				DebugMessage("Error: r"+j+" does not match " + hex8(tracebuffer[tracebufferindex]) + " " + hex8(cpu.r[j]));
				abort();
			}
		...
		{
			DebugMessage("Error: flags does not match " + hex8(tracebuffer[tracebufferindex]) + " " + hex8(cpu.GetFlags()));
			abort();
		}
		tracebufferindex += 4;
		*/
...
for (var i=0;i < str.length; i++) term.PutChar(str.charCodeAt(i));
